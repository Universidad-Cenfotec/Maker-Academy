# Banco de preguntas para desarrollar el pensamiento maker

Este archivo pertenece a: Bloque Temático Pensamiento Maker  
Ruta: 01. Bloques Temáticos/01. Pensamiento Maker/03. Recursos para Docentes/Banco de Preguntas para para desarrollar el pensamiento maker.md

## Estado

**Estado:** Borrador  
**Versión:** v1.0  
**Bloque:** Pensamiento Maker

## Descripción

Este banco reúne preguntas que pueden utilizarse para activar y acompañar el pensamiento maker durante una minilección, práctica guiada, reto o proyecto.

Las preguntas están organizadas según los momentos de XperiencED Kids: Inspiración, Experimentación y Reflexión. También incluyen aspectos relacionados con creatividad, prototipado, colaboración, seguridad, documentación y uso responsable de inteligencia artificial.

No es necesario utilizar todas las preguntas. El docente puede seleccionar, adaptar o simplificar aquellas que respondan mejor al propósito de la experiencia y a las características del grupo.

## Propósito

Apoyar la mediación docente mediante preguntas que promuevan:

- Curiosidad.
- Observación.
- Formulación de problemas.
- Generación de ideas.
- Construcción y prototipado.
- Prueba e iteración.
- Colaboración.
- Documentación.
- Reflexión.
- Autonomía.
- Uso crítico de la tecnología.

## Recomendaciones para utilizar el banco

- Formule una pregunta a la vez.
- Brinde tiempo suficiente para pensar y responder.
- Solicite explicaciones, ejemplos o evidencias.
- Evite convertir las preguntas en un interrogatorio.
- Utilice preguntas abiertas cuando existan varias soluciones posibles.
- Emplee preguntas concretas cuando el equipo necesite enfocar su atención.
- Devuelva preguntas al equipo antes de proporcionar una respuesta.
- Permita responder mediante palabras, dibujos, modelos o demostraciones.
- Adapte el vocabulario a la edad y experiencia del estudiantado.

---

# Fase 1. Inspiración

## Para despertar curiosidad

- ¿Qué observan?
- ¿Qué les llama la atención?
- ¿Qué creen que está ocurriendo?
- ¿Qué les gustaría descubrir?
- ¿Qué parte de esta situación les genera preguntas?
- ¿Han visto algo parecido anteriormente?
- ¿Dónde podemos encontrar este problema en la vida cotidiana?
- ¿Qué pasaría si cambiáramos una de las condiciones?
- ¿Qué objeto, sistema o solución les gustaría comprender mejor?
- ¿Qué creen que existe dentro o detrás de su funcionamiento?

## Para comprender una necesidad

- ¿Cuál es el problema?
- ¿Quién experimenta esta necesidad?
- ¿Cómo afecta a las personas, animales o al ambiente?
- ¿Por qué sería importante buscar una solución?
- ¿Qué necesita realmente la persona usuaria?
- ¿Qué sabemos sobre su contexto?
- ¿Qué podríamos preguntar para comprender mejor?
- ¿Estamos suponiendo algo que deberíamos verificar?
- ¿Todas las personas experimentan el problema de la misma manera?
- ¿Qué condiciones debe respetar una posible solución?

## Para observar soluciones existentes

- ¿Cómo se resuelve actualmente este problema?
- ¿Qué funciona bien en las soluciones existentes?
- ¿Qué podría mejorar?
- ¿Qué materiales o mecanismos utilizan?
- ¿Qué partes son indispensables?
- ¿Qué decisiones parecen haber tomado sus creadores?
- ¿Para quién fue diseñada esta solución?
- ¿Es accesible, segura y fácil de utilizar?
- ¿Qué impacto tiene en el ambiente?
- ¿Qué idea podríamos adaptar sin copiarla completamente?

## Para formular el reto

- ¿Qué queremos lograr?
- ¿Cómo podríamos expresar el reto en una sola frase?
- ¿Qué debe hacer la solución?
- ¿Qué no puede hacer?
- ¿Qué materiales, tiempo o espacio tenemos disponibles?
- ¿Cuáles son los criterios que deberá cumplir?
- ¿Cómo sabremos si la solución funciona?
- ¿Qué aspectos son indispensables y cuáles son opcionales?
- ¿Qué haría que este reto fuera significativo?
- ¿Permite crear diferentes tipos de soluciones?

---

# Fase 2. Experimentación

## Para generar ideas

- ¿Cuántas maneras diferentes imaginan para resolverlo?
- ¿Qué idea parece más sencilla?
- ¿Qué idea parece más original?
- ¿Qué idea podrían probar rápidamente?
- ¿Qué ocurriría si combinaran dos propuestas?
- ¿Cómo lo resolvería una persona con otros materiales?
- ¿Qué solución propondrían si no pudieran utilizar electricidad?
- ¿Qué podrían construir con los materiales disponibles?
- ¿Qué parte de la naturaleza podría inspirar una solución?
- ¿Qué idea parece extraña pero vale la pena explorar?
- ¿Cómo podrían modificar un objeto que ya existe?
- ¿Qué idea responde mejor a las necesidades identificadas?

## Para seleccionar una idea

- ¿Qué criterios utilizarán para decidir?
- ¿Cuál propuesta responde mejor al problema?
- ¿Cuál pueden construir con los recursos disponibles?
- ¿Qué ventajas y limitaciones tiene cada opción?
- ¿Qué idea permite realizar una prueba más rápida?
- ¿Qué riesgo presenta cada alternativa?
- ¿Qué parte todavía necesitan investigar?
- ¿Pueden combinar elementos de varias ideas?
- ¿La propuesta es segura y accesible?
- ¿Cómo justifican la idea seleccionada?

## Para planificar

- ¿Qué necesitan construir primero?
- ¿Qué materiales utilizarán?
- ¿Qué función tendrá cada parte?
- ¿Cómo organizarán el tiempo?
- ¿Qué tarea asumirá cada integrante?
- ¿Qué deben comprobar antes de continuar?
- ¿Qué herramientas requieren supervisión?
- ¿Qué podrían representar primero mediante un dibujo o modelo?
- ¿Qué parte será más difícil?
- ¿Qué harán si la primera idea no funciona?
- ¿Qué evidencia deberán registrar?
- ¿Cómo sabrán cuándo es momento de probar?

## Para construir

- ¿Qué están tratando de lograr en este momento?
- ¿Cómo se conecta esta parte con las demás?
- ¿Qué propiedad del material están aprovechando?
- ¿La estructura se mantiene estable?
- ¿Las piezas pueden moverse como esperaban?
- ¿Qué podrían simplificar?
- ¿Están utilizando los materiales de forma responsable?
- ¿Qué herramienta sería la más adecuada?
- ¿Qué norma de seguridad deben aplicar?
- ¿Qué decisión necesitan tomar antes de continuar?
- ¿Todas las personas del equipo están participando?
- ¿El prototipo representa la idea que planificaron?

## Para probar

- ¿Qué desean comprobar?
- ¿Qué condición mantendrán igual?
- ¿Qué variable modificarán?
- ¿Cómo registrarán el resultado?
- ¿Cuántas veces deberían repetir la prueba?
- ¿Qué esperan que ocurra?
- ¿Qué ocurrió realmente?
- ¿Qué parte funcionó como esperaban?
- ¿Dónde apareció la dificultad?
- ¿El resultado fue siempre el mismo?
- ¿Qué evidencia obtuvieron?
- ¿La prueba representa una situación real de uso?
- ¿La solución cumple con los criterios del reto?
- ¿Es necesario realizar otra prueba antes de concluir?

## Para analizar errores

- ¿En qué momento comenzó el problema?
- ¿Qué información proporciona este error?
- ¿Cuál podría ser la causa?
- ¿Qué cambió desde la última versión?
- ¿Qué parte pueden revisar primero?
- ¿Qué ocurriría si modifican una sola variable?
- ¿El problema está en el diseño, el material, la construcción o la programación?
- ¿Cómo podrían reproducir el fallo?
- ¿Qué evidencia necesitan para confirmar la causa?
- ¿Qué solución temporal podrían probar?
- ¿Qué aprendieron gracias a este intento?
- ¿Existe un error diferente detrás del problema visible?

## Para iterar y mejorar

- ¿Qué cambio tendría mayor impacto?
- ¿Qué parte funciona y conviene conservar?
- ¿Qué parte necesita modificarse?
- ¿Qué pueden mejorar sin reconstruir todo?
- ¿Cómo podrían hacer la solución más sencilla?
- ¿Cómo podrían hacerla más resistente?
- ¿Cómo podrían hacerla más segura?
- ¿Cómo podrían hacerla más accesible?
- ¿Qué comentario recibido resulta útil?
- ¿Qué mejora pueden completar con el tiempo disponible?
- ¿Cómo comprobarán que la nueva versión es mejor?
- ¿Qué diferencia existe entre la primera versión y la actual?
- ¿Qué todavía no han logrado resolver?

## Para programación y robótica

- ¿Qué comportamiento esperan del programa?
- ¿Qué debe ocurrir primero?
- ¿Qué evento inicia la acción?
- ¿Qué instrucción se repite?
- ¿Qué condición debe cumplirse?
- ¿Qué información recibe el sensor?
- ¿Qué respuesta debe ejecutar el robot?
- ¿En qué bloque podría estar el error?
- ¿Qué sucede si prueban una instrucción a la vez?
- ¿El problema está en el código, la conexión o el mecanismo?
- ¿Cómo podrían representar el algoritmo antes de programarlo?
- ¿Qué caso todavía no contempla el programa?
- ¿Cómo podrían hacer el código más claro o eficiente?

## Para fabricación digital

- ¿Qué medidas necesitan verificar?
- ¿El diseño corresponde con el material disponible?
- ¿Qué tolerancia necesitan entre las piezas?
- ¿Cómo se ensamblarán los componentes?
- ¿Qué parte requiere mayor precisión?
- ¿Existe material innecesario que pueda eliminarse?
- ¿El archivo está preparado en el formato correcto?
- ¿Qué riesgos deben considerar antes de fabricar?
- ¿Es necesario hacer una prueba pequeña?
- ¿Cómo podrían reducir desperdicios?
- ¿Qué cambiarían en el diseño antes de fabricar otra versión?

## Para DIY y autonomía

- ¿Qué pueden intentar por cuenta propia?
- ¿Qué conocimiento anterior pueden utilizar?
- ¿Qué herramienta o material necesitan aprender a usar?
- ¿Qué parte pueden explorar antes de pedir ayuda?
- ¿Cómo podrían dividir el problema en pasos?
- ¿Qué recurso podrían consultar?
- ¿Qué decisión corresponde al equipo?
- ¿Qué han descubierto mediante la experimentación?
- ¿Cómo pueden apropiarse y personalizar esta creación?

## Para DIWO y colaboración

- ¿Qué idea aportó cada integrante?
- ¿Cómo están tomando las decisiones?
- ¿Quién todavía necesita una oportunidad para participar?
- ¿Qué tarea pueden realizar en parejas?
- ¿Cómo podrían integrar propuestas diferentes?
- ¿Qué conocimiento puede compartir una persona del equipo?
- ¿Qué pregunta podrían hacerle a otro equipo?
- ¿Cómo pueden ayudar sin construir la solución por otras personas?
- ¿Qué desacuerdo necesita conversar el equipo?
- ¿Qué acuerdo pueden establecer?
- ¿Cómo documentarán los aportes del grupo?
- ¿Qué aprendieron gracias a trabajar con otras personas?

## Para seguridad y responsabilidad

- ¿Qué riesgo observan?
- ¿Qué norma se aplica en esta situación?
- ¿Qué herramienta requiere supervisión?
- ¿Qué equipo de protección necesitan?
- ¿El espacio está ordenado para realizar la prueba?
- ¿Qué podría ocurrir si utilizan incorrectamente este material?
- ¿Cómo pueden reducir el riesgo?
- ¿Es seguro continuar?
- ¿Cuándo deben detenerse y solicitar ayuda?
- ¿Cómo deben guardar o devolver la herramienta?
- ¿Qué impacto ambiental tienen los materiales seleccionados?
- ¿Podrían reutilizar alguna pieza?

## Para documentar el proceso

- ¿Qué decisión importante deberían registrar?
- ¿Qué fotografía mostraría mejor el proceso?
- ¿Qué diferencia existe entre estas dos versiones?
- ¿Qué error conviene documentar?
- ¿Qué aprendieron durante esta prueba?
- ¿Qué información necesitaría otra persona para comprender la creación?
- ¿Qué dibujo, esquema o diagrama podrían incluir?
- ¿Qué evidencia demuestra que la solución funciona?
- ¿Qué cambio realizaron y por qué?
- ¿Cómo organizarán los registros?
- ¿Qué parte fue creada con apoyo de otra persona o herramienta?
- ¿Qué información no debe incluirse por privacidad?

---

# Fase 3. Reflexión

## Para analizar el producto

- ¿Qué construyeron?
- ¿Qué función cumple?
- ¿Qué necesidad busca atender?
- ¿Qué criterios logró cumplir?
- ¿Qué evidencia demuestra que funciona?
- ¿Qué aspecto es más sólido?
- ¿Qué parte todavía necesita mejorar?
- ¿La solución es segura?
- ¿La solución es accesible para las personas usuarias?
- ¿Qué impacto podría tener?
- ¿Qué limitaciones presenta?
- ¿Cómo se compara con la idea inicial?

## Para analizar el proceso

- ¿Cómo comenzó la idea?
- ¿Qué decisiones fueron más importantes?
- ¿Qué cambio produjo la mayor mejora?
- ¿Cuál fue la dificultad principal?
- ¿Qué estrategia utilizaron para superarla?
- ¿Qué error les enseñó algo importante?
- ¿Qué prueba aportó más información?
- ¿Qué parte requirió mayor creatividad?
- ¿En qué momento necesitaron cambiar el plan?
- ¿Qué harían diferente si comenzaran nuevamente?
- ¿Qué habilidad desarrollaron?
- ¿Cómo utilizaron el tiempo y los materiales?

## Para reflexionar sobre la colaboración

- ¿Cómo se organizaron?
- ¿Qué aporte realizó cada integrante?
- ¿Cómo resolvieron los desacuerdos?
- ¿Qué decisión tomaron en conjunto?
- ¿Cuándo necesitaron ayuda?
- ¿Cómo ofrecieron apoyo a otras personas?
- ¿Todas las voces fueron escuchadas?
- ¿Qué podrían mejorar en la organización del equipo?
- ¿Qué aprendieron de otra persona?
- ¿Qué parte habría sido diferente si trabajaban individualmente?

## Para transferir el aprendizaje

- ¿Dónde más podrían utilizar esta idea?
- ¿Qué otro problema podría resolverse con un mecanismo similar?
- ¿Cómo se relaciona con otra asignatura?
- ¿Qué principio científico, matemático o tecnológico utilizaron?
- ¿Cómo podrían adaptar la solución para otra persona?
- ¿Qué cambiarían si tuvieran otros materiales?
- ¿Cómo podría utilizarse en la comunidad?
- ¿Qué nueva pregunta surgió?
- ¿Qué les gustaría aprender después?
- ¿Cómo explicarían este aprendizaje a otro grupo?

---

# Preguntas para el uso responsable de inteligencia artificial

## Antes de utilizar IA

- ¿Para qué necesitan utilizar la herramienta?
- ¿Podrían resolver esta parte sin inteligencia artificial?
- ¿Qué tipo de apoyo esperan recibir?
- ¿Qué información pueden compartir?
- ¿Qué información personal o sensible deben proteger?
- ¿Cómo formularán una solicitud clara?
- ¿Quién tomará la decisión final?

## Al revisar una respuesta generada

- ¿La respuesta se relaciona con el reto?
- ¿Es correcta y comprensible?
- ¿Qué información necesita verificarse?
- ¿Presenta datos, referencias o afirmaciones dudosas?
- ¿Existe algún sesgo o perspectiva ausente?
- ¿La propuesta considera las condiciones reales del proyecto?
- ¿Qué parte deben adaptar?
- ¿Qué parte conviene descartar?
- ¿Cómo pueden comprobar la información mediante otra fuente o prueba?

## Después de utilizar IA

- ¿Cómo influyó la herramienta en el proceso?
- ¿Qué decisiones fueron tomadas por el equipo?
- ¿Qué contenido modificaron?
- ¿Qué aprendieron que antes no sabían?
- ¿La IA sustituyó una experiencia que necesitaban realizar directamente?
- ¿Cómo documentarán su uso?
- ¿Podrían explicar y defender el resultado sin depender de la herramienta?
- ¿Qué limitaciones encontraron?

---

# Adaptación de las preguntas por nivel

## Preescolar

Utilice preguntas breves, acompañadas de objetos, imágenes o acciones:

- ¿Qué ves?
- ¿Qué quieres hacer?
- ¿Cuál pieza necesitas?
- ¿Qué pasó?
- ¿Qué puedes cambiar?
- ¿Quieres probar otra vez?
- ¿Cómo ayudaste al equipo?

## Primero a tercer grado

Promueva explicaciones sencillas:

- ¿Qué quieres construir?
- ¿Por qué elegiste ese material?
- ¿Qué ocurrió en la prueba?
- ¿Qué cambiarás?
- ¿Cómo sabes que funciona?
- ¿Qué aprendiste?

## Cuarto a sexto grado

Incorpore comparación, criterios y documentación:

- ¿Qué criterio están tratando de cumplir?
- ¿Qué evidencia obtuvieron?
- ¿Qué variable podrían cambiar?
- ¿Cómo se diferencia esta versión de la anterior?
- ¿Qué decisión pueden justificar?

## Secundaria

Profundice en análisis, impacto y argumentación:

- ¿Qué evidencia respalda la decisión?
- ¿Cómo controlaron las variables?
- ¿Qué limitaciones tiene el prototipo?
- ¿Qué impacto social, ambiental o ético podría generar?
- ¿Cómo validaron la solución con las personas usuarias?
- ¿Qué mejora deberían priorizar y por qué?
