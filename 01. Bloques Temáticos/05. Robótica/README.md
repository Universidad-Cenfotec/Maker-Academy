# Robótica Educativa

> Ruta: `01. Bloques Temáticos/05. Robótica`

---

## Estado

**Estado:** Completo
**Versión:** v1.2
**Bloque:** 05_robotica

---

> *Insertar próximamente una imagen: fotografía panorámica de un espacio maker con estudiantes trabajando en robots con ruedas sobre una pista de competencia, mostrando distintos niveles de avance — desde robots simples de madera hasta prototipos con sensores.*

---

## Propósito del bloque

La robótica educativa no es enseñar a construir robots. Es usar los robots como excusa para que el estudiante aprenda a **descomponer problemas complejos en partes manejables**, a **iterar cuando algo no funciona** y a **integrar conocimientos de varias áreas** en un objeto que se mueve y responde al mundo real.

Este bloque acompaña al docente en ese camino: desde los conceptos más básicos (¿qué es un robot?) hasta proyectos completos de navegación autónoma y competencia. No asume experiencia técnica previa, y está diseñado para que el docente aprenda junto con sus estudiantes cuando sea necesario.

---

## Estructura de este bloque

```
05. Robótica/
├── README.md                        ← Este archivo
├── 01. Mapa de Progresión.md
├── 02. Vocabulario.md
├── 03. Seguridad.md
├── 04. Alineación con el PNFT.md
│
├── 01. Fundamentos de Robótica/
│   ├── README.md
│   ├── 01. Qué es un robot.md
│   ├── 02. Entrada, proceso y salida en robots.md
│   ├── 03. Chasis, ruedas y estructura.md
│   ├── 04. Sensores y actuadores robóticos.md
│   └── 05. Estabilidad y equilibrio.md
│
├── 02. Movimiento y Mecanismos/
│   ├── README.md
│   ├── 01. Avance y retroceso.md
│   ├── 02. Giros.md
│   ├── 03. Velocidad.md
│   └── 04. Calibración de motores.md
│
├── 03. Sensores y Percepción/
│   ├── README.md
│   ├── 01. Sensor de línea.md
│   ├── 02. Sensor de distancia.md
│   ├── 03. Sensor de luz.md
│   └── 04. Múltiples sensores.md
│
├── 04. SumoBot/
│   ├── README.md
│   ├── 01. Partes del kit y funciones.md
│   ├── 02. Seguridad del kit SumoBot.md
│   ├── 03. Sensores de línea y distancia.md
│   ├── 04. Calibración de sensores.md
│   ├── 05. Progresión de programación del SumoBot.md
│   ├── 06. Mini Lecciones/
│   │   ├── README.md
│   │   ├── 01. El SumoBot se detiene ante un obstáculo.md
│   │   ├── 02. El SumoBot sigue una línea.md
│   │   └── 03. El SumoBot como esquiva-obstáculos.md
│   ├── 07. Proyectos sugeridos con el kit SumoBot.md
│   └── 08. Competencia SumoBot/
│       ├── README.md
│       ├── 01. Reglas básicas.md
│       ├── 02. Dohyo y borde.md
│       ├── 03. Formato y bracket de competencia.md
│       ├── 04. Roles y logística del evento.md
│       ├── 05. Checklist de competencia.md
│       ├── 06. Estrategias de competencia.md
│       ├── 07. Tabla de pruebas de competencia.md
│       └── 08. Mini Lecciones de Competencia/
│           ├── README.md
│           ├── 01. El robot que no debe caerse.md
│           ├── 02. Ojos que sienten al rival.md
│           └── 03. Elige tu estilo de combate.md
│
├── 05. Prácticas Guiadas/
│   ├── README.md
│   ├── 01. Lógica de Programación con Code.org/
│   ├── 02. Robot avanza y se detiene/
│   ├── 03. Robot gira/
│   ├── 04. Sigue línea básico/
│   └── 05. Evita obstáculos básico/
│
└── 06. Evaluación/
    ├── README.md
    ├── 01. Lista de cotejo de robótica.md
    ├── 02. Rúbrica de robótica básica.md
    └── 03. Rúbrica de SumoBot.md
```

---

## Alineación curricular

| Nivel | Enfoque en este bloque |
|---|---|
| Preescolar (4–6 años) | Robots desenchufados: secuencias físicas con el cuerpo |
| Primaria baja (6–9 años) | Primer contacto con robots programables por bloques |
| Primaria alta (9–12 años) | Control de motores, sensores básicos, proyectos guiados |
| Secundaria (12–18 años) | Navegación autónoma, SumoBot, integración de múltiples sensores |

---

## Conexión con XperiencED Kids

| Fase | Cómo se vive en este bloque |
|---|---|
| **Inspiración** | Ver robots en acción, explorar cómo responden al entorno |
| **Experimentación** | Armar, programar, probar, romper, ajustar |
| **Reflexión** | Documentar qué funcionó, qué no y por qué |

---

## Recursos de este bloque

| Recurso | Descripción |
|---|---|
| `01. Mapa de Progresión.md` | Cómo evoluciona el aprendizaje de robótica por nivel |
| `02. Vocabulario.md` | Términos clave con definiciones accesibles |
| `03. Seguridad.md` | Normas de seguridad para el trabajo con robots |
| `04. Alineación con el PNFT.md` | Vínculo con el Programa Nacional de Formación Técnica |
| `01. Fundamentos de Robótica/` | Conceptos base: ¿qué es un robot y cómo funciona? |
| `02. Movimiento y Mecanismos/` | Motores, giros, velocidad y calibración |
| `03. Sensores y Percepción/` | Cómo el robot percibe su entorno |
| `04. SumoBot/` | Guía completa del kit SumoBot: componentes, progresión de programación, proyectos sugeridos y, opcionalmente, competencia |
| `05. Prácticas Guiadas/` | Actividades paso a paso para el aula |
| `06. Evaluación/` | Rúbricas y listas de cotejo por nivel |

---

## Nota docente

No se necesita experiencia en robótica para facilitar este bloque. El rol del docente en este contexto es el de **co-explorador**: alguien que guía el proceso de investigación y ayuda al grupo a reflexionar sobre lo que observa, sin necesitar tener todas las respuestas técnicas de antemano.

Lo más importante es crear un ambiente donde equivocarse sea parte esperada del proceso. Un robot que no avanza en línea recta no es un fracaso: es el punto de partida para que el estudiante entienda por qué los motores necesitan calibración.
